...
- Zod is installed in both apps. Use `z.object(...)` schemas, not `as` casts or `any`.
...
- **Hours vs. paid hours:** `rawHours` is used for coverage and scheduling; `paidHours` is used only for dollar-cost calculation (`apps/api/src/lib/hours.ts`).
...
- Review `CLAUDE.md` and `AGENTS.md` quarterly; move detail to `ARCHITECTURE.md` or `PLAYBOOKS.md` and keep this file under 400 lines.
